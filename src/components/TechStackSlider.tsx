...
const TechStackSlider = ({ 
  ...
  return (
    <div 
      className={`w-full overflow-hidden relative py-8 before:absolute before:left-0 before:z-10 before:w-24 before:h-full before:bg-gradient-to-r before:from-white dark:before:from-black before:to-transparent after:absolute after:right-0 after:z-10 after:w-24 after:h-full after:bg-gradient-to-l after:from-white dark:after:from-black after:to-transparent ${className}`}
      ...
      >
        {allItems.map((item, index) => {
          ...
          return (
            <div 
              key={`${itemName}-${index}`} 
              className={`mx-3 tech-item relative ${variant === "floating" && isActive ? "animate-float" : ""}`}
            ...
            >
              ...
              >
                {variant === "neon" ? (
                  <span className={`flex items-center justify-center w-6 h-6 rounded-full bg-current/20 text-current`}>
                    {item.logo || logoInfo.icon}
                  </span>
                ) : (
                  <span className={`flex items-center justify-center w-6 h-6 rounded-full ${variant === "minimal" ? "" : logoInfo.color} text-white`}>
                    {item.logo || logoInfo.icon}
                  </span>
                )}
